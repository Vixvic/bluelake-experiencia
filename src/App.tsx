...
import ConsolidatedVoucher from "./pages/client/ConsolidatedVoucher";
...
function ProtectedClient({ children }: { children: React.ReactNode }) {
  ...
  if (requiresPasswordChange) return <Navigate to="/update-password" replace />;
  return <>{children}</>;
}
          ...
          <CurrencyProvider>
            <SeasonalProvider>
              <Routes>
                ...
                <Route path="/client/voucher-consolidado" element={<ProtectedClient><ConsolidatedVoucher /></ProtectedClient>} />
